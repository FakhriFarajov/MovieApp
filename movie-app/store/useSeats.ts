...
const useSeatsStore = create<SeatsState>((set, get) => {
  ...
  (async () => {
    try {
      ...
      console.warn('Failed to hydrate seats from AsyncStorage', e);
    }
  })();

  const persist = async (seats: Seat[]) => {
    try {
      await AsyncStorage.setItem(SEATS_KEY, JSON.stringify(seats));
    } catch (e) {
  ...
  return {
    ...
    resetSeats: async () => {
      set({ seats: [] });
      try {
        await AsyncStorage.removeItem(SEATS_KEY);
      } catch (e) {
        console.warn('Failed to clear seats from AsyncStorage', e);
      }
    },
  };
});
