...
export default function TheatresScreen() {
  ...
  const [theatresData, setTheatresData] = useState<Array<Theatre> | null>(null);
  const [theatresLoading, setTheatresLoading] = useState<boolean>(false);
  const [theatresError, setTheatresError] = useState<string | null>(null);

  useEffect(() => {
    const fetchTheatresData = async () => {
      try {
        setTheatresLoading(true);
        var result = await fetchTheatres();
        setTheatresData(result);
      } catch (error) {
    ...
    return (
      ...
      >
        ...
        <View style={styles.cardBody}>
          ...
          {item.halls && item.halls.length > 0 && (
            <View style={styles.hallsContainer}>
              <Text style={[styles.hallsTitle, { color: ((layoutTheme.modes as any)[colorScheme]?.text?.primary) ?? '#fff' }]}>Halls:</Text>
              <View style={styles.hallsList}>
                {item.halls.map((hall) => (
                  <View key={hall.id} style={[styles.hallChip, { backgroundColor: ((layoutTheme.modes as any)[colorScheme]?.button?.accent?.bg) ?? layoutTheme.colors.accent?.[500] ?? '#7c3aed' }]}>
                    <Text style={styles.hallName}>{hall.name}</Text>
                    {hall.rows && hall.columns && (
                      <Text style={styles.hallSeats}>{hall.rows * hall.columns} seats</Text>
                    )}
                  </View>
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: 'row', alignItems: 'flex-start', padding: 12, borderRadius: 12, overflow: 'hidden' },
  mapContainer: { width: 110, height: 84, marginRight: 12, borderRadius: 8, overflow: 'hidden' },
  mapThumb: { flex: 1 },
  ...
  hallsContainer: { marginTop: 10 },
  hallsTitle: { fontSize: 12, fontWeight: '600', marginBottom: 6 },
  hallsList: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  hallChip: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, flexDirection: 'row', alignItems: 'center', gap: 6 },
  hallName: { color: '#fff', fontSize: 12, fontWeight: '600' },
  hallSeats: { color: 'rgba(255,255,255,0.7)', fontSize: 10 },
});
