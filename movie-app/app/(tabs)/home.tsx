...
import { ActivityIndicator, Animated, Dimensions, FlatList, Image, Platform, RefreshControl, SafeAreaView, ScrollView, StatusBar, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
export default function Home() {
  ...
  // fetchGenres may return an array of genres (from backend) or a wrapped { genres: [...] } object.
  const { data: genres, loading, error, refetch: refetchGenres } = useFetch<any[]>(() => fetchGenres());
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await searchMovies({ lang: '', page: 1, pageSize: 50, query: null, genreId: null });
        if (!mounted) return;
  ...
  useEffect(() => {
    ...
    const s = items.slice(0, 12).map((m: any) => {
      const raw = m.backdropPath ?? m.posterPath ?? m.backdrop_path ?? m.poster_path ?? null;
      return {
        uri: raw ? String(raw) : '',
    ...
    >
      <SafeAreaView style={{ flex: 1 }}>
        ...
        >
          ...
          <HeaderBar onNotificationsPress={() => console.log('open notifications')} />
          ...
          <View style={{ alignItems: 'center' }}>
            <Animated.FlatList
              ...
              onScrollToIndexFailed={(info) => {
                // sometimes FlatList can't scroll immediately; retry shortly
                setTimeout(() => {
                ...
                return (
                  <View style={{ width: ITEM_WIDTH, padding: 8, alignItems: 'center', justifyContent: 'center' }}>
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        router.push({
                          pathname: '/movie-details',       // your screen path
                          params: { movie_id: String(src.id) } // send the movie id here
                        });
                      }}
          ...
          <View style={{ marginTop: 12, }}>
                    ...
                    return (
                      <TouchableOpacity key={`latest-${i}`} activeOpacity={0.9} onPress={() => {
                        setActiveIndex(i); indexRef.current = i; try { if (typeof flatRef.current?.scrollToIndex === 'function') { flatRef.current.scrollToIndex({ index: i, animated: true, viewPosition: 0.5 }); } else { flatRef.current?.scrollToOffset({ offset: i * ITEM_WIDTH, animated: true }); } } catch (e) { } finally {
                          router.push({
                            pathname: '/movie-details',       // your screen path
                            params: { movie_id: String(it.id) } // send the movie id here
                          });
                        }
                      }} style={{ marginRight: 12 }}>
                        <View style={styles.latestItem}>
                          {/* Prefer backend-provided raw URLs (posterPath/backdropPath) and do not modify them */}
                          {(() => {
           ...
           {/* NEW: Grid-style movies list (2 columns) similar to Search Results */}
           <View style={{ marginTop: 12 }}>
             <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: '8%' }}>
             ...
             <View style={{ paddingHorizontal: 16, paddingVertical: 12 }}>
               <FlatList
                 ...
                 renderItem={({ item }: { item: Movie }) => {
                   ...
                   const ratingNum = Number((item as any).vote_average ?? (item as any).voteAverage ?? 0);
                 ...
                 ListEmptyComponent={!loadingSlides ? <Text style={styles.placeholderText}>No movies</Text> : <ActivityIndicator color={styles.activityColor.color} />}
               />
             </View>
           </View>
        </ScrollView>
