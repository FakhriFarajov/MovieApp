...
export default function RegisterScreen() {
  ...
  async function handleRegister(values: SignUpSchemaType) {
    try {
      ...
      addUser(newUser);
      await SecureStore.setItem('currentUser', JSON.stringify(newUser));
      await SecureStore.setItem('users', JSON.stringify([...users, newUser]));
      router.replace('/(tabs)/home');
    } catch (error) {
      ...
      <View style={styles.cardWrapper} pointerEvents="box-none">
        <BlurView intensity={35} tint="dark" style={styles.blurCard}>
          <View style={styles.innerCard}>
            ...
            <TouchableOpacity onPress={() => router.push('/auth')} style={styles.linkButton}>
